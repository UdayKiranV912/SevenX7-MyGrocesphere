  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
      ...
      setTimeout(() => {
          if (formData.otp === '1234' || formData.otp === '0000') { 
             loginUser(formData.email, formData.password, formData.fullName, formData.phone)
            ...
            {authMode !== 'VERIFY' && (
                <div className="flex border-b border-slate-100 mb-8 p-1 bg-slate-50 rounded-2xl">
                    <button 
                        ...
                        className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${authMode === 'LOGIN' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}
                    >
                        Log In
                    </button>
                    <button 
                        onClick={() => setAuthMode('REGISTER')}
                        className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${authMode === 'REGISTER' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}
                    >
                        Sign Up
                    </button>
                </div>
                ...
                <div className="animate-fade-in space-y-6">
                    {authMode === 'LOGIN' && (
                        <form onSubmit={handleStandardLogin} className="space-y-4">
                            <input 
                                ...
                                className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-xs font-bold outline-none focus:ring-4 focus:ring-emerald-50 focus:bg-white transition-all"
                                required
                            />
                            <input 
                                type="password" 
                            ...
                            <button type="submit" className="w-full h-14 bg-slate-900 text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] shadow-xl active:scale-[0.98] transition-all">
                                Log In
                            </button>
                        </form>
                    )}
                    ...
                    {authMode === 'REGISTER' && (
                        <form onSubmit={handleRegister} className="space-y-3">
                            ...
                            <button type="submit" className="w-full h-14 bg-slate-900 text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] shadow-xl">
                                Create Account
                            </button>
                        </form>
                    )}
                    ...
                    {authMode === 'VERIFY' && (
                        <div className="text-center space-y-6">
                            <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-[2rem] flex items-center justify-center text-3xl mx-auto border border-emerald-100 shadow-inner">
                                🔐
                            </div>
                            <div>
                            ...
                            <form onSubmit={handleVerifyOTP} className="space-y-4">
                                <input 
                                    ...
                                    className="w-full text-center tracking-[1em] text-2xl font-black bg-slate-50 border border-slate-200 rounded-2xl p-5 outline-none focus:bg-white focus:ring-4 focus:ring-emerald-50 transition-all"
                                    required
                                />
                            ...
                            <button onClick={() => setAuthMode('LOGIN')} className="text-[9px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-colors">Return to Log In</button>
                        </div>
                    )}
                </div>
            )}
        ...
        {authMode !== 'VERIFY' && (
            <div className="bg-slate-50 p-6 text-center border-t border-slate-100">
                <button 
                    type="button" 
                    onClick={onDemoLogin}
                    className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.15em] hover:text-emerald-700 transition-colors flex items-center justify-center gap-2 group mx-auto"
                >
