...
export const ShopPage: React.FC = () => {
  ...
  const availableCategories = useMemo(() => {
    ...
    // In real mode, filter products based on store inventory
  ...
  const filteredProducts = INITIAL_PRODUCTS.filter(p => {
    ...
    const isAvailable = activeStore?.availableProductIds.length === 0 || activeStore?.availableProductIds.includes(p.id);
    ...
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center p-10 text-center animate-fade-in bg-white/50 backdrop-blur-md">
        <div className="relative mb-10">
            <div className="w-40 h-40 bg-slate-50 rounded-[48px] flex items-center justify-center text-7xl shadow-inner border border-slate-100 animate-float">🏬</div>
            <div className="absolute -bottom-2 -right-2 w-12 h-12 bg-emerald-500 rounded-full border-4 border-white flex items-center justify-center text-white text-xl animate-pulse shadow-xl">✨</div>
        </div>
        <h2 className="text-xl font-black text-slate-900 tracking-tight leading-tight mb-4 uppercase">Marts Coming Soon</h2>
        <p className="text-slate-500 font-bold text-[9px] leading-relaxed max-w-[220px] mb-10 mx-auto uppercase tracking-[0.2em]">Verified community marts are joining Grocesphere in your area.</p>
        <button onClick={detectLocation} className="w-full max-w-xs bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-float active:scale-95 transition-all flex items-center justify-center gap-3">📍 Refresh GPS</button>
      </div>
    );
  }
    ...
    <div className="pb-32 animate-fade-in bg-slate-50/30">
      <div className="sticky top-0 z-[50] bg-white/95 backdrop-blur-xl border-b border-slate-100 px-5 py-3 shadow-sm">
        <div className="relative group max-w-md mx-auto">
            <input type="text" placeholder="Search Grocesphere..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full h-11 bg-slate-100 rounded-2xl px-11 text-xs font-bold border-2 border-transparent outline-none focus:ring-4 focus:ring-emerald-50 focus:bg-white transition-all"/>
        ...
        {activeStore && (
            <div className="absolute top-3 left-3 z-[40] animate-slide-up">
              <div className="bg-white/95 backdrop-blur-xl px-4 py-2 rounded-2xl shadow-float border border-white/50 flex items-center gap-2.5">
                  <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] shadow-sm ${activeStore.type === 'produce' ? 'bg-emerald-500 text-white' : activeStore.type === 'dairy' ? 'bg-blue-500 text-white' : 'bg-orange-500 text-white'}`}>
      ...
      <div className="mb-8">
        ...
        <div className="overflow-x-auto hide-scrollbar px-4 flex gap-3">
           {availableCategories.map((family) => (
             <button key={family.id} onClick={() => setSelectedCategory(family.id)} className={`flex items-center gap-2.5 px-5 py-3 rounded-2xl transition-all duration-300 border-2 shrink-0 ${selectedCategory === family.id ? 'bg-slate-900 border-slate-900 text-white shadow-lg -translate-y-0.5' : 'bg-white border-slate-100 text-slate-600 hover:border-slate-300'}`}>
                <span className="text-xl">{family.emoji}</span>
                <span className="text-[10px] font-black uppercase tracking-tight">{family.title}</span>
             </button>
           ))}
        </div>
      </div>
      ...
      <div className="px-5">
        <div className="flex items-center gap-2 mb-5">
             ...
             <h2 className="text-[11px] font-black text-slate-900 uppercase tracking-widest">{PRODUCT_FAMILIES.find(f => f.id === selectedCategory)?.title || 'All Goods'}</h2>
        </div>
